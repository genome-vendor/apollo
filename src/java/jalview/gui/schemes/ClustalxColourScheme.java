...
public class ClustalxColourScheme implements ColourScheme {
  ...
  public void setCons(Hashtable [] cons) {
    this.cons = cons;
  }
  ...
  public Color findColour(DrawableSequence seq, String s, int j) {
    ...
    for (int k=0; k < ResidueColour[i].conses.length ; k++) {
      if (ResidueColour[i].conses[k].isConserved(cons2,j,i,size)) {
        c =  ResidueColour[i].c;
      }
    }
    if (i ==4) {
      if (conses[27].isConserved(cons2,j,i,size)) {
...
//MG      for (int i = 0; i < s.getSequence().length();i++) {
//MG        setColours(s,i);
//MG      }
//MG    }
  }

  public int[][] setCons2(int[][] c2) {
    return cons2 = c2;
  }
